...
from ISTKHAR_MUSIC.utils.errors import capture_internal_err, send_large_error
...
class Call:
    def __init__(self):
        self.userbot1 = Client(
            "ISTKHARXAssis1", config.API_ID, config.API_HASH, session_string=config.STRING1
        ) if config.STRING1 else None
        self.one = PyTgCalls(self.userbot1) if self.userbot1 else None

        self.userbot2 = Client(
            "ISTKHARXAssis2", config.API_ID, config.API_HASH, session_string=config.STRING2
        ) if config.STRING2 else None
        self.two = PyTgCalls(self.userbot2) if self.userbot2 else None

        self.userbot3 = Client(
            "ISTKHARXAssis3", config.API_ID, config.API_HASH, session_string=config.STRING3
        ) if config.STRING3 else None
        self.three = PyTgCalls(self.userbot3) if self.userbot3 else None

        self.userbot4 = Client(
            "ISTKHARXAssis4", config.API_ID, config.API_HASH, session_string=config.STRING4
        ) if config.STRING4 else None
        self.four = PyTgCalls(self.userbot4) if self.userbot4 else None

        self.userbot5 = Client(
            "ISTKHARXAssis5", config.API_ID, config.API_HASH, session_string=config.STRING5
                    ...
                    if popped:
                        try:
                            if await is_autoplay_on(chat_id):
                                from ISTKHAR_MUSIC.utils.stream.autoplay import auto_play_next
                                from ISTKHAR_MUSIC.utils.database import is_active_chat as _is_chat_active
        ...
        async def unified_update_handler(client, update: Update) -> None:
            try:
                ...
                try:
                    ...
                    paste_url = await ISTKHARBIN(full_trace)
                    if paste_url:
                        caption += f"\n🔗 <b>Log:</b> {paste_url}"
                except Exception:
                    pass

...
from ISTKHAR_MUSIC.utils.inline import aq_markup, close_markup, stream_markup
from ISTKHAR_MUSIC.utils.pastebin import ISTKHARBIN
from ISTKHAR_MUSIC.utils.stream.queue import put_queue, put_queue_index
...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await ISTKHAR.join_call(chat_id, original_chat_id, file_path, video=False)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await ISTKHAR.join_call(chat_id, original_chat_id, file_path, video=is_video)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,

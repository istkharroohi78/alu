...
import config
from config import Istu, BANNED_USERS, lyrical
...
_Istu_index = 0
_Istu_order = list(range(len(Istu)))
random.shuffle(_Istu_order)


def get_search_msg(_, channel, user_name=""):
    """Return search message text."""
    global _Istu_index, _Istu_order
    if channel:
        return _["play_2"].format(channel)
    msg = Istu[_Istu_order[_Istu_index]]
    _Istu_index += 1
    if _Istu_index >= len(_Istu_order):
        _Istu_index = 0
        random.shuffle(_Istu_order)
...
async def play_command(
    ...
    mystic_text = get_search_msg(_, channel, message.from_user.mention)
    try:

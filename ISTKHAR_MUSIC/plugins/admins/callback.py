...
from ISTKHAR_MUSIC.utils.decorators import ActualAdminCB, languageCB
from ISTKHAR_MUSIC.utils.formatters import seconds_to_min
from ISTKHAR_MUSIC.utils.inline import close_markup, stream_markup, stream_markup_timer
...
async def manage_callback(client, callback: CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await ISTKHAR.mute_stream(chat_id)
        ...
        await ISTKHAR.unmute_stream(chat_id)
...
async def handle_skip_replay(callback: CallbackQuery, _, chat_id: int, command: str, user_mention: str):
        ...
        try:
            ...
            if not playlist:
                if await is_autoplay_on(chat_id):
                    try:
                        await ISTKHAR.stop_stream(chat_id)
                        from ISTKHAR_MUSIC.utils.database import remove_active_chat, remove_active_video_chat
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await ISTKHAR.skip_stream(chat_id, new_link, video=status, image=image)
        except Exception:

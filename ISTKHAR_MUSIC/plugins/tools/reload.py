...
from ISTKHAR_MUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from ISTKHAR_MUSIC.utils.decorators import AdminActual, language
from ISTKHAR_MUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restart_bot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await ISTKHAR.force_stop_stream(message.chat.id)
    except:
        pass
